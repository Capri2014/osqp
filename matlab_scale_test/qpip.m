function [vars,status,stats] = qpip(Q,c,C,d,A,b,initVars)

% QPIP: Implementation of Mehrotra-like Primal-Dual QP solver
% for general quadratic programs
%
% Usage:
%   [vars,status,stats] = qpip(Q,c,C,d,A,b)   
%
...
%
%
% Author : P. Goulart, Univ. Oxford.  Last modified 17/07/2017

%a bunch of constants
MAXITER  = 100;
GAMMA    = 0.99;

%Allow for inequalities only
if(~exist('A','var') || isempty(A))
    A = []; b = [];
end

%make the complete problem structure 
%using the input arguments
p = initializeProblem(Q,c,C,d,A,b);

%data norm initialize
normD = dataNorm(p.data,inf);

%use a default first guess unless
%one is already provided
if(~exist('initVars') || isempty(initVars))
    p = defaultStart(p,normD);
else
    p.variables = initVars;
end

%get the complementarity measure mu
muval = calcMu(p);

%supress factorization warnings
...
stats.status     = status;
stats.value      = 1/2*(vars.x'*(Q*vars.x)) + c'*vars.x;
...
%Print some problem information 

fprintf('------------------------------------------------------------\n');
fprintf('::: QP interior point solver :::\n');
fprintf('Author                : P. Goulart\n');
...
if(muval <= tolMu & normR <= tolR*normD) 
    %convergence.  Exit.
...
%------------------------------------------------------------
%------------------------------------------------------------
function p = initializeProblem(Q,c,C,d,A,b)

%Get a full set of arguments
if(~exist('A')),A =[];b=[]; end

%construct 0-dimensional equality constraint
%if none is specified (NB: 0x1 matrix != [])
if(isempty(A))
    A = zeros(0,size(C,2));
    b = zeros(0,1);
end
...
p.data      = initializeData(Q,c,A,b,C,d);
...
%sparsify everything
[Q,A,C] = deal(sparse(Q),sparse(A),sparse(C));
...
%------------------------------------------------------------
%------------------------------------------------------------
function data = initializeData(Q,c,A,b,C,d)
